...
constraint models.  Currently there are three constraint models named
``minus_z``, ``psmc``, and ``pline``.  The easiest way to understand the output
is to run the ``plot_by_month.py`` script as shown and then start examining the
``constraints`` variable.
::

  >>> print constraints
  {'minus_z': <pyger.pyger.ConstraintMinusZ object at 0x1c9d2ad0>, 
   'all': <pyger.pyger.ConstraintModel object at 0x19db97d0>, 
   'psmc': <pyger.pyger.ConstraintPSMC object at 0x1c628bd0>, 
   'pline': <pyger.pyger.ConstraintPline object at 0x1c6384d0>}

So there are four elements in the dictionary, each of which is a
ConstraintModel object.  In addition to the three actual constraints 
